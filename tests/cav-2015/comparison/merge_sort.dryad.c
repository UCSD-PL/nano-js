...
Node * g_slist_sort_merge(Node * l1, Node * l2)
  /*D_requires sortedlist^(l1) * sortedlist^(l2) */
  /*D_ensures (sortedlist^(ret) & (keys^(ret) s= (old(keys^(l1)) union old(keys^(l2))))) */
{
	Node * list, * l, * y;  
  ...
  Node * list_next =list->next;
    ...
    /*D_invariant (((((sortedlist^(l1) * sortedlist^(l2)) * ((list |> loc next: y) * sortedlist^(y))) 
                    & ((((keys^(l1) union keys^(l2)) union keys^(y)) s= (old(keys^(l1)) union old(keys^(l2))) * true)) 
                    & (((y l= nil) & (l l= x)) | (((sortedlseg^(y, l) & (lsegkeys^(y, l) setlt kl)) * ((l |> loc next: nill; int key: kl) & (nill l= nil))) * true))) & (((keys^(y) lt keys^(l1)) * true) 
