...
/*@ is_black :: forall A.
                (x:{v:<t>+null | ((Prop(nil(v))) <=> (Prop(nil(isin))))})/t |-> isin:rbtree[A]<{\x y -> x > y},{\x y -> x < y}>
                  => {v:boolean | (((Prop v) <=> ((~Prop(nil(isin))))) && (col(isin) = 0))}/t |-> isout:{v:rbtree[A]<{\x y -> x > y},{\x y -> x < y}>  | ((Prop(nil(v)) <=> Prop(nil(isin))) && (bheight(v) = bheight(isin)) && (col(v) = col(isin)))} */
// function is_black(x) 
// {
//   if (x == null) 
//   {
...
/*@
  rbalS :: forall A.
    (t:<t>)/ r |-> rbr:rbtree[{v:A | v > field_int(rbt, "key")}]<{\x y -> x > y },{\x y -> x < y }>
         ...
         => <x>/x |-> out:{v:rbtree[A]<{\x y -> x > y },{\x y -> x < y }> | (if (col(rbr) != 0) then (if (col(rbl) != 0) then ((col(v) = 0) && (bheight(v) = bheight(rbl) + 1)) else ((col(v) != 0) && (bheight(v) = bheight(rbl)))) else (if (col(rbl) != 0) then ((col(v) = 0) && (bheight(v) = bheight(rbl) + 1)) else (bheight(v) = bheight(rbl))))}
*/
// function rbalS(t) 
// {
//   var l = t.left;
