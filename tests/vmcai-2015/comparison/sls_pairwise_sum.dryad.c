...
Node * sls_pairwise_sum(Node * x, Node * y)
  /*D_requires (sortledsll^(x) * sortedsll^(y)) */
  /*D_ensures ((sortledsll^(x) * sortedsll^(y)) * sortedsll^(z)) */
{
  if (x == NULL || y == NULL) {
    return NULL;
  } 
  ...
  while(curr_x->next != NULL && curr_y->next != NULL)
    /*D_invariant (((((((curr_x |> int key: cxk) * true) & sortedsll^(curr_x)) * (sortedlseg^(x, curr_x) & (lsegkeys^(x, curr_x) setle cxk)))
                    * ((((curr_y |> int key: cyx) * true) & sortedsll^(curr_y)) * (sortedlseg^(y, curr_y) & (lsegkeys^(y, curr_y) setle cyk))))
                   * (((last_z |> loc next: nil; int key: lzk) * true) & (sortedlseg^(z, last_z) & (lsegkeys^(z, last_z) setle  lzk))))
                  & (lzk i= cxk + cyk)) */
  {
    Node * tmp = last_z;
    curr_x = curr_x->next;
