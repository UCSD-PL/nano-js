...
define relation lseg^(head, tail): 
  ...
  ( (sll^(tail) -* (sll^(head) & (keys^(head) s= (lseg-keys^(head, tail) union keys^(tail))))) &
    (((tail |-> loc next: virtual tn; int key: virtual tk) * sll^(tn)) -* ((lseg^(head, tn) & (lseg-keys^(head, tn) s= (lseg-keys^(head, tail) union (singleton tk)))) * sll^(tn))) ) ) ;//73
...
define relation sorted-lseg^(head, tail): 
  ...
  ( (sorted-sll^(tail) -* ((lseg-keys^(head, tail) le keys^(tail)) => sorted-sll^(head))) &
    (((tail |-> loc next: virtual tn; int key: virtual tk) * sll^(tn)) -* (((lseg-keys^(head, tail) set-le tk) * true) => (sorted-lseg^(head, tn) * sll^(tn)))) ) ) ) ;//52
...
SNnode * concat_sorted(SNnode * l1, SNnode * l2)
  /*D_requires (sortedsll^(l1) * sortedsll^(l2)) */
  /*D_ensures  (sortedsll^(ret) & (keys^(ret) s= (old(keys^(l1)) union old(keys^(l2))))) */
;

_(dryad)
SNnode * quick_sort(SNnode * l)
  /*D_requires sll^(l) */
  /*D_ensures (sortedsll^(ret) & (keys^(ret) s= old(keys^(l)))) */ 
{
