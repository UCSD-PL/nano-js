...
Node * sls_double_all(Node * lst)
  /*D_requires sortedsll^(lst) */
  /*D_ensures  (sortedsll^(lst) * sortedsll^(ret)) */
{
  if (lst == NULL) {
    return NULL;
  } 
  ...
  while(curr>next != NULL) 
    /*D_invariant (((((curr |> int key: ky) * true) & sortedsll^(lst)) 
                   & ((sortedlseg^(lst, curr) & (lsegkeys^(lst, curr) setle ky)) * sortedsll^(curr))) 
                 * ((sortedsll^(res) & (((cp |> loc next: nil; int key: ck) * true) & (ck i= (ky + ky)))) 
                    & (sortedlseg^(res, cp) & (lsegkeys^(res, cp) setle (ky + ky))) * sortedsll^(cp)))  */
  {
    Node * old_cp = cp;
